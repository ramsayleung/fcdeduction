...
class IdentificationDao : public fcdeduction::dao::BaseDao {
   public:
    ...
    // 通过IdentKey和IdentType查询用户Id
    std::optional<std::string> findUserIdByIdentKeyAndIdentType(const std::string &tntInstId, const std::string &identKey, const fcdeduction::util::IdentTypeEnum &identType);
    // 新增识别记录
